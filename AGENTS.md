# AGENTS.md - soia-open-dev-design-skills
...
`soia-open-dev-design-skills` publishes reusable `soia-dev-*` skills for the dev domain. Every committed skill must be safe for users who do not share the maintainer's machine, accounts, private data, or internal workspace.
## Routing Boundary
...
- Put user-specific behavior behind CLI args, env vars, or skill-specific
  ...
  `~/.config/soia-skills/soia-open-dev-design-skills/<skill-type>/<skill-name>/config.yml`.
...
```bash
npx skills add soia-team/soia-open-dev-design-skills -l --full-depth
npx skills add soia-team/soia-open-dev-design-skills -g --all
```
...
```bash
claude plugin update soia-dev-design@soia   # 交付走插件市场，勿装全局
```

Forbidden outside local testing:

```bash
npx skills add /absolute/local/path/to/soia-open-dev-design-skills -g -a '*' -s <skill-name> -y
```
...
```bash
cd <your-local-checkout>/soia-open-dev-design-skills
...
3. **Users update** with `claude plugin update soia-dev-design@soia` or
...
claude plugin update soia-dev-design@soia
```
